...
export default {
  ...
  getters: {
    ...
    currentQuestionRef(state) {
      return state.assessments[state.questions.currentAssessment].questions[
    ...
    currentQuestion(state) {
      if (state.questions.currentAssessment > 0) {
        ...
        for (var i = state.questions.currentAssessment - 1; i >= 0; i--) {
          previousAssessmentTotal +=
            state.questions.assessments[i].questions.length;
        }
    ...
    pageProgressPercent(state, getters) {
      return Math.floor(
        (getters.currentQuestion / getters.totalQuestions) * 100
      );
    }
  },
  mutations: {
    ...
    setAssessment(state, progressForward = true) {
      if (progressForward) {
        ...
        state.questions.currentAssessment -= 1;
        state.questions.currentQuestion =
          state.questions.assessments[state.questions.currentAssessment]
            .questions.length - 1;
      }
    },
    answerQuestionAndAdvance(state, response) {
      ...
      if (
        ...
        state.questions.assessments[currentAssessment].questions.length
      ) {
        state.questions.currentQuestion += 1;
      } else {
        if (
          state.questions.currentAssessment + 1 <
          state.questions.assessments.length
        ) {
  ...
  actions: {
    loadAssessments(context) {
      ...
      let url = `https://www.mocky.io/v2/${
        context.state.login.currentUser.userId
      }`;
      axios
        .get(url)
        .then(response => {
        ...
        .catch(e => {
          ...
          console.log('error at loadAssessments')
          // eslint-disable-next-line
          console.log(e)
          context.commit(
            "setError",
            "An error occurred while loading your assessments, this is most likely due to an interruption of service"
          );
        })
        .finally(() => {
